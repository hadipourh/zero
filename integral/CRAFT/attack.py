...
class ZC:
    ...
    def __init__(self, RB, RU, RL, RF, \
        ...
        self.mzn_file_name = "cpkeyrecoveryrtk.mzn"
    ...
    def search(self):
        ...
        if self.result.status == minizinc.Status.OPTIMAL_SOLUTION or self.result.status == minizinc.Status.SATISFIED or \
            ...
            if self.RF + self.RB > 0:
                self.max_num_of_involved_key_cells = self.result["critical_path"]
    ...
    def draw_ef(self, r):
        ...
        targeted_positions = [k for k in range(16) if any(self.result["kxf"][k][0])]
        state_before_mc = [self.result["kxf"][k][r] for k in targeted_positions]
        if r <= self.RF - 1:
            state_after_permutation = [self.result["kxf"][k][r + 1] for k in targeted_positions]
        else:
            state_after_permutation = [self.result["kxf"][k][r] for k in targeted_positions]
        # check if the size of targeted_positions is at most 2 otherwise raise and error
        if len(targeted_positions) > 2:
            raise Exception("The size of targeted_positions is greater than 2")        
        if len(targeted_positions) == 2:
            for i in range(16):
                if state_before_mc[0][i] == 1:
                    before_mc += "\TFill{{s{0}}}".format(i)
                if state_after_permutation[0][i] == 1:
                    after_permutation += "\TFill{{s{0}}}".format(i)
                    after_mc += "\TFill{{s{0}}}".format(self.p_permutation[i])
                    after_addtk += "\TFill{{s{0}}}".format(self.p_permutation[i])
                    round_tk += "\TFill{{s{0}}}".format(self.p_permutation[i])                       
            for i in range(16):
                if state_before_mc[1][i] == 1:
                    before_mc += "\BFill{{s{0}}}".format(i)
                if state_after_permutation[1][i] == 1:
                    after_permutation += "\BFill{{s{0}}}".format(i)
                    after_mc += "\BFill{{s{0}}}".format(self.p_permutation[i])
                    after_addtk += "\BFill{{s{0}}}".format(self.p_permutation[i])
                    round_tk += "\BFill{{s{0}}}".format(self.p_permutation[i])
        else:
            for i in range(16):
                if state_before_mc[0][i] == 1:
                    before_mc += "\Fill[blue!55]{{s{0}}}".format(i)
                if state_after_permutation[0][i] == 1:
                    after_permutation += "\Fill[blue!55]{{s{0}}}".format(i)
                    after_mc += "\Fill[blue!55]{{s{0}}}".format(self.p_permutation[i])
                    after_addtk += "\Fill[blue!55]{{s{0}}}".format(self.p_permutation[i])
                    round_tk += "\Fill[blue!55]{{s{0}}}".format(self.p_permutation[i]) 
        return before_mc, after_mc, round_tk, after_addtk, after_permutation
    ...
    def draw_graph(self):
        ...
        contents += r"""%\caption{ZC/Integral attack on """ +  str(self.num_of_attacked_rounds) + " rounds of CRAFT"\
                    r""". Twakeys cells that are active at most 1 time: """ + ", ".join(self.lazy_tweak_cells) + \
...
def parse_args():
    ...
    parser.add_argument("-RL", "--nroundsE2", default=9, type=int, help="choose the number of rounds for E2\n")
